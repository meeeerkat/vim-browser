...
namespace TabsWidget {
    ...
    void init(void (*update_page_display_callback) (Page*))
    {
        ...
        current_tab_index = 0;
        update_page_display_callback = update_page_display_callback;
    ...
    void update_view()
    {
        ...
        for (uint32_t i=0; i < get_pages_nb(); i++) {
            std::string *title = pages[i].get_title();
            if (title == NULL)
                mvwprintw(window, 0, i*tab_size, "%hhu: Loading...", i);
            else
                mvwprintw(window, 0, i*tab_size, "%hhu: %s", i, title->c_str());
        }
    ...
    void close_tab(uint8_t tab_index)
    {
        ...
        else if(current_tab_index == tab_index && current_tab_index == get_pages_nb()) {
            current_tab_index--;
